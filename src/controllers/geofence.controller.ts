...
export class GeofenceController {
    ...
    try {
      ...
      const [geofences, total] = await Promise.all([
        prisma.$queryRaw`
          ...
          ${
            req.user.role !== "admin"
              ? prisma.$queryRaw`WHERE g.company_id = ${req.user.company_id}::uuid`
              : prisma.$queryRaw``
          }
          ORDER BY g.${prisma.$queryRawUnsafe(sort)} ${prisma.$queryRawUnsafe(
          order
        )}
  ...
  static async findById(req: AuthRequest, res: Response) {
    try {
      ...
      const params =
        req.user.role !== "admin" ? [id, req.user.company_id] : [id];
      ...
      if (geofence_id) {
        ...
        result = await prisma.$queryRaw`
          SELECT 
            g.id,
            g.name,
            ST_Contains(g.geom, ST_SetSRID(ST_MakePoint(${
              location.longitude
            }, ${location.latitude}), 4326)) as inside
          FROM geofences g
          WHERE g.id = ${geofence_id}::uuid
          ${
            req.user.role !== "admin"
              ? prisma.$queryRaw`AND g.company_id = ${req.user.company_id}::uuid`
              : prisma.$queryRaw``
          }
        `;
      } else {
        ...
        result = await prisma.$queryRaw`
          ...
          FROM geofences g
          WHERE ST_Contains(g.geom, ST_SetSRID(ST_MakePoint(${
            location.longitude
          }, ${location.latitude}), 4326)) = true
          ${
            req.user.role !== "admin"
              ? prisma.$queryRaw`AND g.company_id = ${req.user.company_id}::uuid`
              : prisma.$queryRaw``
